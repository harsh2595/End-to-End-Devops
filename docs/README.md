# Docs Index

This folder contains the stage walkthroughs for the project.

## Stage Guides

- [Stage 1: Local Node.js Development](stage-1-local-node.md)
- [Stage 2: Docker Compose](stage-2-docker-compose.md)
- [Stage 3: Terraform AWS Infrastructure](stage-3-terraform-aws.md)
- [Stage 4: Kubernetes Deployment](stage-4-kubernetes.md)
- [Stage 5: Helm Packaging](stage-5-helm.md)
- [Stage 6: CI/CD Pipelines](stage-6-cicd.md)

## Stage 5 Support Files

- [Stage 5 command sequence](../command-sequence/stage5-command-sequence.md)
- [Stage 5 file structure](../file-structure/stage5-file-structure.md)
- [Helm chart scaffold](../helm/end-to-end-devops/)

## Stage 6 Support Files

- [.github/workflows/terraform-provision.yml](../.github/workflows/terraform-provision.yml)
- [.github/workflows/deploy.yml](../.github/workflows/deploy.yml)
- [Stage 6 final destroy script](stage6-final-destroy.sh)
- [Stage 6 command sequence](../command-sequence/stage6-command-sequence.md)
- [Stage 6 file structure](../file-structure/stage6-file-structure.md)
- [Stage 6 GitHub settings steps](stage6-github-steps.md)
- [Stage 6 pre-flight checklist](stage6-preflight-checklist.md)
- [Stage 6 GitHub IAM policies](stage6-github-policy/README.md)

## Screenshot Folders

- [Stage 1 screenshots](../screenshots/stage1%20-%20ss/)
- [Stage 2 screenshots](../screenshots/stage2-ss/)
- [Stage 3 screenshots](../screenshots/stage3-ss/)
- [Stage 4 screenshots](../screenshots/stage4-ss/)
- [Stage 5 screenshots](../screenshots/stage5-ss/)
- [Stage 6 screenshots](../screenshots/stage6-ss/)
